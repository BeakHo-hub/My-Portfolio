document.addEventListener('DOMContentLoaded', () => {
    ...
    const profileData = {
        ...
        sprite: 'images/character-sprite.png',
        stats: [
        ...
        contacts: [
            { type: 'email', link: 'mailto:you@example.com', icon: 'images/icon-email.png' },
            { type: 'github', link: 'https://github.com/BeakHo-hub', icon: 'images/icon-github.png' }
        ]
    };
    ...
    coin.addEventListener('mousedown', e => { 
        ...
        window.addEventListener('mousemove', dragCoin); 
        window.addEventListener('mouseup', dropCoin); 
    });
    function dragCoin(e) { 
        if (!dragging) return; 
        const parentRect = coin.parentElement.getBoundingClientRect();
        coin.style.left = `${e.clientX - parentRect.left - offX}px`; 
        coin.style.top = `${e.clientY - parentRect.top - offY}px`; 
    }
    function dropCoin() { 
        if (!dragging) return; 
        dragging = false; 
        coin.classList.remove('dragging'); 
        window.removeEventListener('mousemove', dragCoin); 
        window.removeEventListener('mouseup', dropCoin); 
        const c = coin.getBoundingClientRect(), s = coinSlot.getBoundingClientRect(); 
        const hit = !(c.right < s.left || c.left > s.right || c.bottom < s.top || c.top > s.bottom); 
        if (hit) {
            insertCoin();
        } else { 
            coin.style.left = '5%'; 
    ...
    function populateProfile() {
        ...
        statsG.innerHTML = '';
        contacts.innerHTML = '';
        const typeWriter = (el, txt, spd = 50) => {
            let i = 0;
            const cur = document.createElement('span');
            cur.className = 'profile-name-cursor';
            el.textContent = '';
            el.appendChild(cur);
            const t = setInterval(() => {
                if (i < txt.length) {
        ...
        setTimeout(() => {
            ...
            }, profileData.name.length * 100 + 500);
